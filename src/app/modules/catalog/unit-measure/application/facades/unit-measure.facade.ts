...
export class UnitMeasureFacade {
  private readonly repository = inject(UnitMeasureRepository);

  readonly selected = signal<UnitMeasure | null>(null);
  readonly filters = signal<UnitMeasureSearchable>(UnitMeasureSearchableDefault);
  ...
  load() {
    this.filters.set(UnitMeasureSearchableDefault);
  }

  updateFilters(newFilters: Partial<UnitMeasureSearchable>) {
  ...
  async create(payload: CreateUnitMeasure): Promise<ApiResponse<UnitMeasure>> {
    try {
  ...
  async update(unitMeasure: UnitMeasure): Promise<ApiResponse<UnitMeasure>> {
    try {
      const response = await firstValueFrom(this.repository.update(unitMeasure));
      this.resource.value.update((current) => {
        if (!current) return current;
        return {
          ...current,
          data: {
            ...current.data,
            content: current.data.content.map((u) => (u.id === unitMeasure.id ? response.data : u)),
          },
        };
      });
      return response;
  ...
  async enable(id: UnitMeasure['id']): Promise<ApiResponse<void>> {
  ...
  async disable(id: UnitMeasure['id']): Promise<ApiResponse<void>> {
    this.setProcessing(id, true);
    try {
  ...
  private updateStatus(id: UnitMeasure['id'], active: boolean) {
    this.resource.value.update((current) => {
      if (!current) return current;
      return {
        ...current,
        data: {
          ...current.data,
          content: current.data.content.map((u) => (u.id === id ? { ...u, active } : u)),
        },
      };
    });
  }
