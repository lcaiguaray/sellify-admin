...
@Component({
  ...
  template: `
    <app-page-header
      ...
      [actionLabel]="canWrite() ? 'Nueva Unidad' : ''"
    ...
    <ng-template #nameCell let-unitMeasure>
      <div class="flex flex-col">
        ...
        <span class="text-xs text-muted-foreground">
          {{ unitMeasure.code }} · {{ unitMeasure.symbol }}
        </span>
      </div>
    </ng-template>
...
export default class ListPage implements OnInit {
  ...
  readonly canWrite = computed(() => this.auth.hasPermission('UOM.WRITE'));
  readonly canManage = computed(() => this.auth.hasPermission('UOM.MANAGE'));
