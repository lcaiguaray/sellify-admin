import { Component, computed, inject, OnInit, TemplateRef, viewChild } from '@angular/core';
import { FormsModule } from '@angular/forms';
import { NgIcon, provideIcons } from '@ng-icons/core';
import { hugeDelete04, hugeDownload03 } from '@ng-icons/huge-icons';
import { tablerDots, tablerLoader2 } from '@ng-icons/tabler-icons';
import { toast } from '@spartan-ng/brain/sonner';
import { mapHttpErrorToUiState, parseHttpError } from '@core/utils/http-error.util';
import { DataTableComponent } from '@ui/data-table/data-table';
import { TableColumn } from '@ui/data-table/data-table.model';
import { ListToolbar } from '@ui/list-toolbar';
import { PageHeader } from '@ui/page-header';
import { Pagination } from '@ui/pagination';
import StatusBadge from '@ui/status-badge';
import TableRowSkeleton from '@ui/table-row-skeleton';
import { HlmButtonImports } from '@ui-spartan/button';
import { HlmCardImports } from '@ui-spartan/card';
import { HlmDialogService } from '@ui-spartan/dialog';
import { HlmDropdownMenuImports } from '@ui-spartan/dropdown-menu';
import { HlmTableImports } from '@ui-spartan/table';
import { UserFacade } from '../../../application/facades/user.facade';
import { User } from '../../../domain/models/user.model';
import { UserFormDialog } from '../../components/user-form-dialog';
import { AuthFacade } from '@modules/auth';

@Component({
  selector: 'app-user-list-page',
  imports: [FormsModule, NgIcon, HlmButtonImports, HlmCardImports, HlmDropdownMenuImports, HlmTableImports, DataTableComponent, ListToolbar, PageHeader, Pagination, StatusBadge, TableRowSkeleton],
  providers: [provideIcons({ hugeDelete04, hugeDownload03, tablerDots, tablerLoader2 })],
  template: `
    <app-page-header
      class="mb-4"
      title="Usuarios"
      description="Gestione las cuentas con acceso a la empresa"
      [actionLabel]="canWrite() ? 'Nuevo usuario' : ''"
      (action)="openForm()"
    />
    <app-list-toolbar class="mb-4" [searchTerm]="filters().search" (searchChange)="search($event)">
      <button hlmBtn variant="outline"><ng-icon name="hugeDownload03" /> Exportar</button>
    </app-list-toolbar>
    <hlm-card size="sm" class="mb-4 w-full p-0">
      <div hlmCardContent class="p-0">
        @if (isLoading()) {
          <div hlmTableContainer><app-table-row-skeleton [rows]="5" [columns]="5" /></div>
        } @else {
          <app-data-table [data]="users()" [columns]="columns()" [error]="uiError()" />
        }
      </div>
    </hlm-card>
    <ng-template #identityCell let-user>
      <div class="flex flex-col">
        <span class="font-medium">
          {{ user.identity.businessName || ((user.identity.firstName || '') + ' ' + (user.identity.lastName || '')) || user.username }}
        </span>
        <span class="text-xs text-muted-foreground">{{ user.identity.documentType.name }} · {{ user.identity.taxId }}</span>
      </div>
    </ng-template>
    <ng-template #contactCell let-user>
      <div class="flex flex-col">
        <span>{{ user.identity.email || 'Sin correo' }}</span>
        <span class="text-xs text-muted-foreground">{{ user.identity.phone || 'Sin teléfono' }}</span>
      </div>
    </ng-template>
    <ng-template #statusCell let-user><app-status-badge [status]="user.active" /></ng-template>
    <ng-template #actionsCell let-user>
      <button hlmBtn size="icon" variant="outline" [hlmDropdownMenuTrigger]="actionsMenu" [disabled]="processingIds().has(user.id)">
        <ng-icon [name]="processingIds().has(user.id) ? 'tablerLoader2' : 'tablerDots'" [class.animate-spin]="processingIds().has(user.id)" />
      </button>
      <ng-template #actionsMenu>
        <hlm-dropdown-menu class="w-44">
          <button hlmDropdownMenuItem variant="destructive" (click)="toggle(user)">
            <ng-icon name="hugeDelete04" /> {{ user.active ? 'Deshabilitar' : 'Habilitar' }}
          </button>
        </hlm-dropdown-menu>
      </ng-template>
    </ng-template>
    <app-pagination
      [pageSize]="pagination().pageSize"
      [currentPage]="pagination().pageNumber + 1"
      [totalPages]="pagination().totalPages"
      (pageChanged)="facade.changePage($event - 1)"
      (pageSizeChanged)="facade.changePageSize($event)"
    />
  `,
})
export default class UserListPage implements OnInit {
  protected readonly facade = inject(UserFacade);
  private readonly dialogs = inject(HlmDialogService);
  private readonly auth = inject(AuthFacade);
  readonly canWrite = computed(() => this.auth.hasPermission('USER.WRITE'));
  readonly canManage = computed(() => this.auth.hasPermission('USER.MANAGE'));
  readonly users = this.facade.data;
  readonly filters = this.facade.filters;
  readonly isLoading = this.facade.isLoading;
  readonly pagination = this.facade.pagination;
  readonly processingIds = this.facade.processingIds;
  readonly uiError = computed(() => mapHttpErrorToUiState(this.facade.error()));
  private readonly identityTemplate = viewChild.required<TemplateRef<any>>('identityCell');
  private readonly contactTemplate = viewChild.required<TemplateRef<any>>('contactCell');
  private readonly statusTemplate = viewChild.required<TemplateRef<any>>('statusCell');
  private readonly actionsTemplate = viewChild.required<TemplateRef<any>>('actionsCell');
  readonly columns = computed<TableColumn<User>[]>(() => {
    const columns: TableColumn<User>[] = [
      { key: 'identity', label: 'Persona / Empresa', customTemplate: this.identityTemplate() },
      { key: 'username', label: 'Usuario' },
      { key: 'contact', label: 'Contacto', customTemplate: this.contactTemplate() },
      { key: 'status', label: 'Estado', align: 'center', customTemplate: this.statusTemplate() },
    ];
    if (this.canManage()) {
      columns.push({ key: 'actions', label: 'Acciones', align: 'center', customTemplate: this.actionsTemplate(), tdClass: 'w-24' });
    }
    return columns;
  });

  ngOnInit(): void { this.facade.load(); }
  search(value: string): void { this.facade.updateFilters({ search: value }); }
  openForm(): void { this.dialogs.open(UserFormDialog, { disableClose: true }); }
  async toggle(user: User): Promise<void> {
    try {
      const response = user.active ? await this.facade.disable(user.id) : await this.facade.enable(user.id);
      toast.success(response.message);
    } catch (error) {
      toast.error(parseHttpError(error));
    }
  }
}
