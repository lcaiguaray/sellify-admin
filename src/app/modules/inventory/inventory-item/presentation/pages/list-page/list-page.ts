...
import { InventoryFacade } from '@modules/inventory/inventory-item/application/facades/inventory.facade';
import { FormsModule } from '@angular/forms';
import { InventoryCreateDialog } from '../../components/inventory-form-dialog';
import { HlmDialogService } from '@ui-spartan/dialog';
import { Pagination } from '@ui/pagination';
import InventoryTable from '../../components/inventory-table';
import { InventoryItem } from '../../../domain/models/inventory.model';
import { InventoryFractionateDialog } from '../../components/inventory-fractionate-dialog';
  ...
  template: `
    <div class="flex flex-col gap-4 mb-4 md:flex-row md:items-center md:justify-between">
      <div>
        ...
        <div>Gestione el inventario de sus productos</div>
      </div>
          ...
          <table hlmTable>
            <thead hlmTableHeader class="bg-muted/50">
              <tr hlmTableRow>
                ...
                <th hlmTableHead>Stock Mínimo</th>
                <th hlmTableHead>Almacén</th>
            ...
            <tbody hlmTableBody>
              @if (facade.isLoading()) {
                ...
                <app-inventory-table 
                  [data]="facade.data()" 
                  (edit)="onOpenFormDialog($event)" 
                  (fractionate)="onOpenFractionateDialog($event)" 
                />
              }
            </tbody>
          </table>
...
export default class ListPage implements OnInit {
  ...
  onOpenFormDialog(item?: InventoryItem) {
    this.dialogService.open(InventoryCreateDialog, {
      context: { inventoryItem: item ?? null },
      disableClose: true,
    });
  }

  onOpenFractionateDialog(item: InventoryItem) {
    this.dialogService.open(InventoryFractionateDialog, {
      context: { inventoryItem: item },
      disableClose: true,
    });
  }
}
